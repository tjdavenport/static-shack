...
function listPages(projectPath) {
  return new Promise((resolve, reject) => {
    fs.readdir(projectPath, { withFileTypes: true }, (err, dirents) => {
    ...
    return new Promise((resolve, reject) => {
      if (page.__name === 'default') {
        ...
        fs.mkdir(path.join(destinationPath, page.__name), err => {
          if (err) reject(err);
          page.__writePath = path.join(destinationPath, page.__name, page.__name+'.html');
          page.__rendered = ejs.render(layouts[page.layout], {
            ...page, 
            content: ejs.render(page.html, page),
          });
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const pages = await Promise.all(pageNames.map(name => {
        return buildPage(projectConfig, path.join(projectPath, name));
      }));
