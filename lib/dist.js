...
function buildAssets(projectPath) {
  return fsp.readdir(path.join(projectPath, 'assets'))
    .then(filenames => {
      return Promise.all(filenames.map(filename => {
        return fsp.readFile(path.join(projectPath, 'assets', filename), 'utf8')
          .then(data => {
...
module.exports = function(projectPath, destinationPath) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      ]).then(([ rawConfig, layouts, pageNames,  assets]) => { 
        return { 
      ...
      await fsp.mkdir(path.join(destinationPath, 'assets'));
      await Promise.all(assets.map(asset => {
        return fsp.writeFile(path.join(destinationPath, 'assets', asset.filename), asset.data)
      }));
